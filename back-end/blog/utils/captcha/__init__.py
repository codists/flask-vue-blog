...
class Captcha:
    ...
    @staticmethod
    def _generate_random_color(start=0, end=255):
        """生成随机的颜色"""
        return random.randint(start, end), random.randint(start, end), random.randint(start, end)

    def _generate_line(self, draw, width, height):
        """绘制干扰线"""
        begin = (random.randint(0, width), random.randint(0, height))
        end = (random.randint(0, width), random.randint(0, height))
        draw.line([begin, end], fill=self._generate_random_color(), width=2)

    def _generate_points(self, draw, point_chance, width, height):
        """绘制干扰点"""
        ...
        for w in range(width):
            for h in range(height):
                ...
                if tmp > 100 - chance:
                    draw.point((w, h), fill=self._generate_random_color())

    @staticmethod
    def _generate_random_font():
        fonts = [
            'LHANDW.TTF',
            'Lobster-Regular.ttf',
            'verdana.ttf'
        ]
        return str(BASEDIR / f'blog/utils/captcha/font/{random.choice(fonts)}')
...
>>>>>>> eefe7407c7923dffc54bc0cbeb6520794efe3fa2
        ...
        draw.text(((width - font_width) / 2, (height - font_height) / 2), text, font=font,
                  fill=self._generate_random_color(150, 255))  # 填充字符
        for _ in range(0, self.line_number):  # 绘制干扰线
            self._generate_line(draw, width, height)
        self._generate_points(draw, 10, width, height)  # 绘制干扰点
        return image
